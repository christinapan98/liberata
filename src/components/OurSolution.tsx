        ...
        <>
            ...
            >
                <OpenCloseChildren
                    ...
                    oldSystemContent= {`Traditionally, academic credit is given in the form of authorship positions on papers. However, these positions can only give an ordering of contributions (>, <), but not exact distances in contributions. These lead to scenarios where it is impossible to tell if one author did 10% or 10x more work than another, and this results in highly exploitable metrics whereby the same credit is given to all authors, or only to the first author, both of which are inaccurate in almost all cases. These metrics can then cause politics and abuse of power dynamics in the assignment of authorship positions.`}
                    liberataContent="Liberata uses contribution shares to assign academic credit, similar to percentage ownership of patents or companies. These shares have no direct financial value, and are used for a new suite of more accurate academic metrics based on shares and weighted citations. Optionally, institutions may refer to the share split on papers when assigning IP ownership, which can give the shares indirect financial value. The share based system allows the precise quantification of contributions, rather than just the ordering of contributions. Liberata will use a default blind survey mechanism to assign shares to circumvent power dynamics issues, but authors are allowed to decide their own share split in lieu of a survey."
